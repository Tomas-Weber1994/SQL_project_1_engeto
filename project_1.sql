...
CREATE OR REPLACE TABLE t_tomas_weber_project_SQL_secondary_final AS (
	...
	FROM economies e
	WHERE 
		`year` IN (SELECT DISTINCT year_of_measurement FROM t_tomas_weber_project_SQL_primary_final) /* stejné období jako pøehled pro ÈR výše */
		AND country IN (SELECT DISTINCT country FROM countries) /* Výbìr pouze státù, v tabulce economies jsou totiž i celé oblasti dohromady */
